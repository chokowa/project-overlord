...
export function updateHudDisplay() {
    ...
    const bossUnits = (window.activeEnemies || []).filter(e => e.isActive && e.tier.id === 'BOSS');

    if (bossUnits.length > 0 && bossContainer && bossFill) {
        bossContainer.classList.remove('hidden');
        const totalMax = bossUnits.reduce((sum, b) => sum + b.maxHealth, 0);
        const totalCur = bossUnits.reduce((sum, b) => sum + b.health, 0);
        bossFill.style.width = `${Math.max(0, (totalCur / totalMax) * 100)}%`;
