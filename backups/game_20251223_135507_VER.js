 ...
 */
import { 
    GAME_SETTINGS, GEMS, GEM_TYPES, ARTIFACTS, ARTIFACT_TYPES, 
    BOSS_ARTIFACTS, MISC_ITEMS, UNIQUES, SHOP_ITEMS, ENEMY_TIERS, 
    STAGE_CONFIG, BOSS_WAVES, UI_STRINGS, CREW_DATA, SKILL_TREE_NODES 
} from './constants.js';
import { generateUuid, createItemInstance } from './utils.js';
import { FloatingText, ParticleEffect, ShockwaveEffect } from './effects.js';
...
Object.assign(GameEngine.prototype, {
    ...
    spawnStageBoss() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            totalHp += boss.maxHealth;
        }
        this.bossWaveMaxHp = totalHp;
...
// 配列は同期ミスの防止とUI参照のため、engineState内で一元管理されます
let activeEnemies = [];
...
class EnemyUnit {
            ...
            if (canMove) {
                if (this.tier.id === 'BOSS') {
                    if (this.bossState === 'ENTER') {
                        ...
                        if (this.positionY >= 180) this.bossState = 'FIGHT';
                    } else {
                        // [Patch] Boss Anchor Logic (Prevent knockback OOB)
                        // UI被り防止のため位置を下げ、強制的にY=180付近に戻す
                        const targetY = 180;
    ...
    draw(context) {
        ...
        // 6. Health Bar
        if (this.tier.id === 'BOSS') {
            // ボスのHPは固定UIで表示するため、個別のHPバーは描画しない
            context.restore();
            return;
        }
        context.shadowBlur = 0;
...
function mainLoop() {
    ...
    if (!engineState.isPaused) {
        ...
        engineState.activeEnemies.forEach(enemy => {
            enemy.update();
            engineState.activeProjectiles.forEach(projectile => {
        ...
        engineState.activeEnemies = engineState.activeEnemies.filter(enemy => enemy.isActive);
        engineState.activeProjectiles = engineState.activeProjectiles.filter(projectile => projectile.isAlive);
