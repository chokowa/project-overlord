...
export function updateCrewHud() {
    ...
    if (!container) {
        ...
        container.style.cssText = "position:absolute; bottom:200px; right:10px; display:flex; flex-direction:column; gap:10px; pointer-events:none; z-index:45;";
        document.getElementById('device-frame').appendChild(container);
    }

    // Clear and Rebuild (Simple enough for 2 items)
    container.innerHTML = '';
    ...
    engineState.selectedCrew.forEach(crewId => {
        ...
        // ボス戦時の警告エフェクト
        const borderColor = engineState.isBossWave ? '#e74c3c' : '#444';
        const shadow = engineState.isBossWave ? '0 0 15px red' : '0 0 5px #000';
        ...
        // 画像パス生成: Character/1 + a + .png
        // suffix: a(通常), b(被弾), c(ピンチ)
        ...
        img.style.imageRendering = 'pixelated'; // ドット絵用

        // ピンチ時は点滅させる
        if (engineState.crewStatusSuffix === 'c') {
            img.style.animation = 'pulse 1s infinite';
        }
        // 被弾時はシェイク (CSS transform)
        ...
        wrapper.onmouseenter = (e) => window.showTooltip(e, { name: crew.name, desc: crew.job, color: '#fff' });
...
export function showCrewSelection(onComplete) {
    ...
    // フッターボタン（あとで決める）を非表示
    const footerBtn = modal.querySelector('.upgrade-btn:last-of-type');
    if (footerBtn && footerBtn.getAttribute('onclick').includes('closeUpgradeModal')) {
...
export function renderSkillTree() {
    ...
    Object.values(SKILL_TREE_NODES).forEach(node => {
        ...
        if (max > 1) {
            ...
            badge.style.cssText = `
                position:absolute; bottom:-18px; width:140%; left:-20%; 
                text-align:center; font-size:11px; color:#fff; font-weight:bold;
                background:rgba(0,0,0,0.85); border-radius:10px; padding:2px 0;
