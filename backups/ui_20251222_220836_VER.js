...
export function refreshInventoryInterface() {
    ...
    subRow.innerHTML = `<div class="loadout-label">SUB<br><span style="color:#f39c12">50% Power</span></div>`;
    ...
    const sortedInv = [...engineState.inventory].sort((a, b) => {
        ...
        return b.level - a.level; // High level first
    });
    ...
    sortedInv.forEach(item => {
        ...
        let icon = '?';
        if (item.type === GEM_TYPES.ACTIVE) icon = '⚔️';
        else if (item.type === GEM_TYPES.SUPPORT) icon = '💎';
        else if (item.type === ARTIFACT_TYPES.RING) icon = '💍';
        else if (item.type === ARTIFACT_TYPES.AMULET) icon = '🧿';

        slot.innerHTML = `<span style="font-size:16px;">${icon}</span><span style="font-size:8px; color:#fff; margin-top:2px;">Lv.${item.level}</span>`;
        if (item.name) slot.title = item.name;
        ...
        slot.ondblclick = () => {
            if (item.type === GEM_TYPES.ACTIVE) {
                // Try Main 0, then Sub 0
                if (!engineState.equippedGems[0]) engineState.equipItem(item.uuid, 'MAIN_0');
                else if (!engineState.altGems[0]) engineState.equipItem(item.uuid, 'SUB_0');
                else engineState.equipItem(item.uuid, 'MAIN_0'); // Default swap main
            } 
            else if (item.type === GEM_TYPES.SUPPORT) {
                // Try Main 1, 2 then Sub 1, 2
                if (!engineState.equippedGems[1]) engineState.equipItem(item.uuid, 'MAIN_1');
                else if (!engineState.equippedGems[2]) engineState.equipItem(item.uuid, 'MAIN_2');
                else if (!engineState.altGems[1]) engineState.equipItem(item.uuid, 'SUB_1');
                else if (!engineState.altGems[2]) engineState.equipItem(item.uuid, 'SUB_2');
                else engineState.equipItem(item.uuid, 'MAIN_1');
            }
            else if (item.type === ARTIFACT_TYPES.RING) engineState.equipItem(item.uuid, 'RING');
            else if (item.type === ARTIFACT_TYPES.AMULET) engineState.equipItem(item.uuid, 'AMULET');
        };
    ...
    salvage.style.cssText = "margin-top:15px; border:2px dashed #e74c3c; color:#e74c3c; padding:10px; text-align:center; font-size:12px; cursor:default;";
    salvage.innerText = "ここに捨てて XP に変換";
