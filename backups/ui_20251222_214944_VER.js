...
export function refreshInventoryInterface() {
    ...
    // Update Slots
    const updateSlot = (id, item, label, typeHint) => {
        ...
        if (item) {
            ...
            el.innerText = label;
            // Keep the type-hint border color if empty
    ...
    engineState.equippedGems.forEach((g, i) => updateSlot(`slot-${i}`, g, i === 0 ? 'メイン' : '改造', i === 0 ? 'ACTIVE' : 'SUPPORT'));
