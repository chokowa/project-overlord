...
export function updateHudDisplay() {
    ...
    if (engineState.isBossWave && engineState.bossWaveMaxHp > 0 && bossContainer && bossFill) {
        const bossUnits = engineState.activeEnemies.filter(e => e.tier.id === 'BOSS');
        if (bossUnits.length > 0) {
            bossContainer.classList.remove('hidden');
            const totalCur = bossUnits.reduce((sum, b) => sum + Math.max(0, b.health), 0);
            const pct = (totalCur / engineState.bossWaveMaxHp) * 100;
