...
Object.assign(GameEngine.prototype, {
    ...
    activateCrewAbility(crewId) {
        ...
        if (crew.id === 1 || crew.id === 2) {
            ...
            // Xeno: Nano Injection
            const maxHP = GAME_SETTINGS.BASE_MAX_HP + this.stats.hp_max;
            const heal = maxHP * 0.20;
            this.baseIntegrity = Math.min(this.baseIntegrity + heal, maxHP);

            // Refill Shield Energy
            this.energy = this.maxEnergy;

            activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y, `HP +${Math.floor(heal)} / MP MAX`, "#2ecc71", 30));
        }
        else if (crew.id === 5) {
            // Mida: Bribe
            let count = 0;
            activeEnemies.forEach(e => {
                ...
                if (count < 5) {
                    ...
                    this.gold += gold;
                    activeFloatingTexts.push(new FloatingText(e.positionX, e.positionY, `+${gold}G`, "#f1c40f", 20));
                    count++;
                }
            });
        }
