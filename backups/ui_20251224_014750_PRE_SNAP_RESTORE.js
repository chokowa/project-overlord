...
export function updateCrewHud() {
    ...
    // 描画先をコックピットパネル内のモニターに変更
    let container = document.getElementById('crew-monitor-grid');
    if (!container) return;

    // 初期プレースホルダーがある場合はクリア
    if (container.children.length > 0 && !container.querySelector('[id^="crew-icon-"]')) {
        container.innerHTML = '';
    }
    ...
    engineState.selectedCrew.forEach(crewId => {
        ...
        if (!wrapper) {
            ...
            // 通信モニター風スタイルに変更（高さは親グリッドに追従、幅を調整）
            wrapper.style.cssText = `
                width: 60px; height: 100%; background: #000; 
