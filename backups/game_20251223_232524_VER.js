...
class EnemyUnit {
    ...
    draw(context) {
        ...
        // 6. Health Bar & Numeric HP Display
        context.shadowBlur = 0;
        const isBoss = this.tier.id === 'BOSS';
        const hpBarW = isBoss ? 80 : 40; // ボスはバーを長く
        const hpBarH = isBoss ? 6 : 4;   // ボスはバーを太く
        const hpY = this.positionY - halfSize - 15;
        ...
        if (isBoss) {
            ...
            context.lineWidth = 3;
            context.strokeText(displayHp, this.positionX, hpY - 5);

            // 数値本体
            context.fillStyle = "#fff";
            context.fillText(displayHp, this.positionX, hpY - 5);
        }
