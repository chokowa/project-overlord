 ...
 */
import { GAME_SETTINGS, GEMS, GEM_TYPES, ARTIFACTS, ARTIFACT_TYPES, BOSS_ARTIFACTS, MISC_ITEMS, UNIQUES, SHOP_ITEMS, ENEMY_TIERS, STAGE_CONFIG, BOSS_WAVES, UI_STRINGS } from './constants.js';
...
class GameEngine {
    constructor() {
        ...
        this.shieldTimer = 0;
        this.shieldImpactTimer = 0; // Visual effect for impact
    }
...
class EnemyUnit {
    ...
    takeDamage(damage, isCritical, sourceId) {
            ...
            if (sourceId === 'fireball' && artifacts.some(a => a.id === 'oil_flask')) {
                // Apply extra burn or damage immediately
                finalDamage *= 1.5; // Direct hit boost
                // Note: Burn DoT boost is handled in applyStatus logically, 
                // but here we just boost the trigger damage which scales the DoT.
            }
            ...
            if (this.freezeTimer > 0 && artifacts.some(a => a.id === 'zero_crystal')) {
                ...
                if (this.tier.id !== 'BOSS' && Math.random() < 0.1) {
                    finalDamage = this.health + 999; // Ensure kill
            ...
            if (this.freezeTimer <= 0 && sourceId !== 'burn_dot') {
                // [Patch] Knockback Resistance for Tanks
                if (this.tier.id === 'TANK') {
                    this.positionY -= 1; // Heavy resistance
                } else if (this.tier.id !== 'BOSS') {
                    this.positionY -= 5; // Standard knockback
                }
            } 

            // [Patch] Synergy Damage Colors
            let popupColor = EFFECT_CONSTANTS.COLOR_NORMAL;
            if (sourceId === 'steam') popupColor = "#dff9fb";        // Cyan-White
            else if (sourceId === 'poison_blast') popupColor = "#be2edd"; // Toxic Purple
            else if (sourceId === 'electric') popupColor = "#f6e58d";     // Bright Yellow
            else if (sourceId === 'meltdown') popupColor = "#fab1a0";     // Pale Red
            else if (sourceId === 'overload') popupColor = "#ff5252";     // Intense Red
            else if (sourceId === 'superconduct') popupColor = "#a29bfe"; // Ice Purple
            else if (sourceId === 'corrosion') popupColor = "#7fff00";    // Acid Green
            else if (sourceId === 'plague') popupColor = "#9b59b6";       // Plague Purple
            else if (sourceId === 'glacier') popupColor = "#74b9ff";      // Deep Ice
            else if (sourceId === 'shield_bash') popupColor = "#66fcf1";  // Shield Cyan
            ...
            // Big font for synergies
            const synergyIds = [
                'steam','poison_blast','electric',
                'meltdown','overload','superconduct',
                'corrosion','plague','glacier'
            ];
...
window.startGame = function() {
    ...
    // Prevent multiple loops if clicked multiple times
