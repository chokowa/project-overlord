...
export function showGameClearScreen() {
    ...
    title.innerHTML = "🎉 MISSION COMPLETE 🎉";
    title.style.cssText = "font-size: 28px; font-weight: bold; color: #f1c40f; margin-bottom: 20px; text-shadow: 0 0 15px #f1c40f;";
    ...
    timeDiv.innerHTML = `CLEAR TIME: <span style="color:#00d2d3">${mins}m ${secs}s</span>`;
    timeDiv.style.marginBottom = "20px";
    overlay.appendChild(timeDiv);

    // --- 装備一覧 (Main, Sub, Artifacts) ---
    const eqSection = document.createElement('div');
    eqSection.style.cssText = "width:100%; max-width:500px; margin-bottom:20px; background:rgba(255,255,255,0.05); padding:10px; border-radius:8px;";
    eqSection.innerHTML = `<div style="border-bottom:1px solid #444; margin-bottom:10px; font-size:14px; color:#aaa;">FINAL LOADOUT</div>`;

    const eqFlex = document.createElement('div');
    eqFlex.style.cssText = "display:flex; justify-content:center; gap:10px; flex-wrap:wrap;";

    // Helper: Icon Generator
    const createIcon = (item, borderColor) => {
        const el = document.createElement('div');
        el.style.cssText = `width:40px; height:40px; border:2px solid ${borderColor}; border-radius:4px; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; font-size:20px; position:relative;`;
        let icon = '?';
        if (item.type === 'ACTIVE') icon = '⚔️';
        else if (item.type === 'SUPPORT') icon = '💠';
        ...
        if (item.level) {
            ...
            lv.innerText = item.level;
            lv.style.cssText = "position:absolute; bottom:0; right:0; font-size:10px; background:#000; padding:0 2px; color:#fff;";
            el.appendChild(lv);
        }
        return el;
    };
    ...
    // Main Gems
    engineState.equippedGems.forEach(g => {
        if(g) eqFlex.appendChild(createIcon(g, g.type==='ACTIVE'?'#c0392b':'#27ae60'));
    });
    // Sub Gems
    engineState.altGems.forEach(g => {
        if(g) {
            const el = createIcon(g, g.type==='ACTIVE'?'#d35400':'#27ae60');
            el.style.opacity = "0.7"; // Dim for sub
            eqFlex.appendChild(el);
        }
    });
    // Artifacts
    engineState.artifacts.forEach(a => {
        eqFlex.appendChild(createIcon(a, a.color));
    });

    eqSection.appendChild(eqFlex);
    overlay.appendChild(eqSection);

    // --- DPS Report ---
    const statsSection = document.createElement('div');
    statsSection.style.cssText = "width:100%; max-width:500px; margin-bottom:20px;";
    statsSection.innerHTML = `<div style="border-bottom:1px solid #444; margin-bottom:10px; font-size:14px; color:#aaa;">COMBAT STATISTICS</div>`;

    const table = document.createElement('table');
    table.style.cssText = "width:100%; font-size:12px; border-collapse:collapse;";
    table.innerHTML = `
        <tr style="color:#888; text-align:left;"><th>SKILL</th><th style="text-align:right">TOTAL DMG</th><th style="text-align:right">DPS</th></tr>
    `;

    // Sort logs by damage
    const sortedLogs = Object.entries(engineState.damageLog).sort(([,a], [,b]) => b - a);

    sortedLogs.forEach(([srcId, totalDmg]) => {
        // Resolve Name
        let name = srcId;
        const gem = Object.values(GEMS).find(g => g.id === srcId);
        if (gem) name = gem.name;
        else if (srcId === 'shield_bash') name = "シールドバッシュ";
        else if (srcId === 'reflected_orb') name = "反射弾";

        const dps = (totalDmg / totalTimeSec).toFixed(1);
        const dmgStr = Math.floor(totalDmg).toLocaleString();

        const tr = document.createElement('tr');
        tr.style.borderBottom = "1px solid #333";
        tr.innerHTML = `
            <td style="padding:4px 0; color:${gem ? gem.color : '#fff'}">${name}</td>
            <td style="text-align:right; color:#ddd;">${dmgStr}</td>
            <td style="text-align:right; color:#f1c40f; font-weight:bold;">${dps}</td>
        `;
    ...
    chartSection.style.cssText = "width:100%; max-width:500px; margin-bottom:20px;";
    chartSection.innerHTML = `<div style="border-bottom:1px solid #444; margin-bottom:10px; font-size:14px; color:#aaa;">WAVE PROGRESS</div>`;

    const canvas = document.createElement('canvas');
    canvas.width = 500;
    canvas.height = 150;
    canvas.style.cssText = "width:100%; height:auto; background:#111; border:1px solid #333; border-radius:4px;";
    chartSection.appendChild(canvas);
    overlay.appendChild(chartSection);

    // Draw Chart
    const ctx = canvas.getContext('2d');
    const logs = engineState.waveLog; // [{wave:1, time:10}, ...]
    if (logs.length > 0) {
        const padding = 20;
        ...
        logs.forEach((log, i) => {
            const x = padding + (log.time / maxTime) * w;
            const y = canvas.height - padding - ((i + 1) / logs.length) * h;
            ctx.lineTo(x, y);
        ...
        ctx.fillText("START", padding, canvas.height - 5);
        ctx.fillText("CLEAR", w, canvas.height - 5);
    }

    // --- Return Button ---
    const btn = document.createElement('button');
    btn.innerText = "タイトルに戻る";
    btn.style.cssText = "padding:15px 40px; background:#e74c3c; color:#fff; border:none; border-radius:4px; font-size:16px; cursor:pointer; font-weight:bold; margin-bottom:30px;";
