...
import { FloatingText, ParticleEffect, ShockwaveEffect, StarField, BeamEffect } from './effects.js';
...
class GameEngine {
    constructor() {
        ...
        // [Hardcore] Heat Management System
        this.heat = 0;
        this.maxHeat = 100;
        this.heatPerShot = 16.0; // Heat gained per shot (adjusted: 2.5 → 8.0 → 16.0)
        this.coolingRate = 0.4; // Passive cooling per frame (adjusted: 0.8 → 0.4)
        ...
        this.isCoolingDown = false; // Rapid cooling, no heat gain
...
Object.assign(GameEngine.prototype, {
    ...
    completeWaveOrGame() {
        ...
        if (this.currentWaveNumber === 10) {
            // Milestone Reached: Show Result but continue!
            this.waveLog.push({ wave: "MILESTONE_10", time: (Date.now() - this.startTime) / 1000 });
            this.isPaused = true; // Pause game for result screen

            if (window.showGameClearScreen) {
                // Show screen but don't stop game logic (it's an overlay)
                setTimeout(() => {
                    window.showGameClearScreen();
                    activeFloatingTexts.push(new FloatingText(GAME_SETTINGS.SCREEN_WIDTH / 2, GAME_SETTINGS.SCREEN_HEIGHT / 2 + 120, "ENDLESS MODE START!", "#e74c3c", 40));
                }, 500); // Shorten delay since we are pausing
            }
            // Do NOT return; continue to standard wave progression
        }
    ...
    updateHeat() {
        // [Hardcore] Overdrive & Cooldown Logic

        // 1. Cooldown Phase (Cooling down rapidly, no heat gain)
        if (this.isCoolingDown) {
            this.cooldownTimer--;
            this.heat -= 2.0; // Rapid cooling
            if (this.heat < 0) this.heat = 0;

            // Visual effect for cooling
            if (Math.random() < 0.2) {
                activeParticles.push(new ParticleEffect(RENDER_CONSTANTS.TURRET_POS_X + (Math.random() - 0.5) * 40, RENDER_CONSTANTS.TURRET_POS_Y + (Math.random() - 0.5) * 40, "#3498db", 4));
            }

            if (this.cooldownTimer <= 0) {
                this.isCoolingDown = false;
                activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 100, "SYSTEM READY", "#66fcf1", 24));
                audioManager.play('RECHARGE');
            }
            return; // Skip normal cooling/overdrive check
        }

        // 2. Overdrive Phase (Max heat locked, damage bonus active)
        if (this.isOverdrive) {
            this.heat = this.maxHeat; // Lock at max
            this.overdriveTimer--;

            // Visual effect for Overdrive
            if (Math.random() < 0.3) {
                activeParticles.push(new ParticleEffect(RENDER_CONSTANTS.TURRET_POS_X + (Math.random() - 0.5) * 50, RENDER_CONSTANTS.TURRET_POS_Y + (Math.random() - 0.5) * 50, "#e74c3c", 6));
            }
            ...
            if (this.overdriveTimer <= 0) {
                ...
                audioManager.play('POWER_DOWN'); // Uses generic sound if not exists
            }
            return;
        }
        ...
        if (this.heat >= this.maxHeat && !this.isBeamActive) {
            ...
            this.overdriveTimer = 300; // 5 seconds duration

            triggerScreenShake(15, 10);
            activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 100, "OVERDRIVE!!", "#e74c3c", 32));
            audioManager.play('POWER_UP');

            for (let i = 0; i < 20; i++) {
                activeParticles.push(new ParticleEffect(RENDER_CONSTANTS.TURRET_POS_X, RENDER_CONSTANTS.TURRET_POS_Y, "#e74c3c", 8));
            }
        }
        ...
        if (this.isBeamActive) {
            ...
            if (this.heat <= 0) {
                ...
                activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 80, "BEAM DEPLETED", "#f39c12", 18));
                return;
            }

            const heatRatio = this.heat / this.maxHeat;
            const baseDamage = 3; // Lower per-frame damage for continuous beam
            const beamWidth = 120 + heatRatio * 80; // 120-200px width
            const beamLength = 800 + heatRatio * 400; // 800-1200px length (reaches boss)
            const damage = baseDamage * (1 + heatRatio);
                ...
                if (rotatedX > 0 && rotatedX < beamLength && Math.abs(rotatedY) < beamWidth / 2) {
                    ...
                    if (isCritical && Math.random() < 0.05) {
                        ...
                        if (!enemy.statusEffects.burn) {
                            enemy.statusEffects.burn = { duration: 180, damage: 5 };
                        }
                    }
                }
            });
            ...
            if (Math.random() < 0.15) {
                for (let i = 0; i < 3; i++) {
                    ...
                    const py = RENDER_CONSTANTS.TURRET_POS_Y + Math.sin(beamAngle) * dist + Math.cos(beamAngle) * offset;
                    activeParticles.push(new ParticleEffect(px, py, blastColor, 12));
                }
            }
        }
    },
    ...
    triggerHeatBlast() {
        // [Hardcore] Toggle continuous beam on/off
        const minHeatRequired = 50;
        ...
        if (this.isBeamActive) {
            ...
            // Try to turn on beam
            if (this.heat < minHeatRequired) {
                activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 80, "NOT ENOUGH HEAT", "#f39c12", 18));
                return;
            }

            // Cancel Overdrive/Cooldown logic if active
            this.isOverdrive = false;
            this.overdriveTimer = 0;
            this.isCoolingDown = false;
            this.cooldownTimer = 0;
            ...
            const heatRatio = this.heat / this.maxHeat;
            const isCritical = heatRatio >= 0.9;
            const blastText = isCritical ? "CRITICAL BEAM!" : "BEAM ON!";
            const blastColor = isCritical ? "#ff6b6b" : "#f39c12";
            activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 100, blastText, blastColor, isCritical ? 32 : 28));
    ...
    toggleFiring() {
        this.isFiringEnabled = !this.isFiringEnabled;
        const statusText = this.isFiringEnabled ? "FIRING ON" : "CEASEFIRE";
        const statusColor = this.isFiringEnabled ? "#2ecc71" : "#e74c3c";
        activeFloatingTexts.push(new FloatingText(RENDER_CONSTANTS.TURRET_POS_X, GAME_SETTINGS.CASTLE_Y - 100, statusText, statusColor, 24));
        audioManager.play('CLICK');
    },
...
window.addEventListener('keydown', (e) => {
    ...
    // [Hardcore] V key for Heat Blast
    if (e.code === 'KeyV') {
        if (!engineState.isPaused && !engineState.isGameOver) engineState.triggerHeatBlast();
    }
    // [Hardcore] F key for Fire Toggle
    if (e.code === 'KeyF') {
        if (!engineState.isPaused && !engineState.isGameOver) engineState.toggleFiring();
    }
});
...
function handleCanvasInput(event) {
    ...
    // [Hardcore] Turret click for Fire Toggle
    const turretClickRange = 80; // Horizontal range
    const turretVerticalRange = 50; // Vertical range
    const inTurretX = Math.abs(clickX - RENDER_CONSTANTS.TURRET_POS_X) < turretClickRange;
    const inTurretY = Math.abs(clickY - RENDER_CONSTANTS.TURRET_POS_Y) < turretVerticalRange;
    if (inTurretX && inTurretY) {
        engineState.toggleFiring();
...
// [Patch] Aegis Barrier Draw Function (Extracted for clarity or integrated into EnemyUnit.draw)
// Since EnemyUnit.draw handles enemy visualization, we will patch that instead.
...
class EnemyUnit {
    constructor(tier = ENEMY_TIERS.NORMAL, xOverride = null, yOverride = null) {
        ...
        if (tier.id !== 'BOSS') {
            waveMultiplier = Math.pow(1.10, Math.max(0, engineState.currentWaveNumber - 1));
        }
        ...
        // [Patch] Aegis Barrier Setup
        if (this.tier.id === 'AEGIS') {
            this.isBarrierActive = true;
            // Wave 1-4: 1 hit, 5-8: 2 hits, 9+: 3 hits
            if (engineState.currentWaveNumber <= 4) this.barrierHp = 1;
            else if (engineState.currentWaveNumber <= 8) this.barrierHp = 2;
            else this.barrierHp = 3;

            // Adjust spawn position to center for Aegis
            this.positionX = GAME_SETTINGS.SCREEN_WIDTH / 2;
        }
    }
    ...
    checkSynergy() {
        ...
        if (this.burnTimer > 0 && this.soakedTimer > 0) {
            ...
            this.burnTimer = 0;
            this.soakedTimer = 0;
            let steamDmg = (this.maxHealth * 0.15);
            if (this.tier.id === 'BOSS') steamDmg = Math.min(steamDmg, 3000);
            steamDmg += 150;
        ...
        if (this.burnTimer > 0 && this.poisonStacks > 0) {
            ...
            let blastDmg = (this.maxHealth * 0.05);
            if (this.tier.id === 'BOSS') blastDmg = Math.min(blastDmg, 1000);
            blastDmg += (stacks * 80) + 100;
        ...
        if (this.burnTimer > 0 && this.freezeTimer > 0) {
            ...
            this.burnTimer = 0;
            this.freezeTimer = 0;
            let meltDmg = (this.maxHealth * 0.25);
            if (this.tier.id === 'BOSS') meltDmg = Math.min(meltDmg, 5000); // Boss cap
            meltDmg += 300;
        ...
        if (this.burnTimer > 0 && this.shockTimer > 0) {
            ...
            let overloadDmg = (this.maxHealth * 0.05);
            if (this.tier.id === 'BOSS') overloadDmg = Math.min(overloadDmg, 1000);
            overloadDmg += 100;
        ...
        if (this.freezeTimer > 0 && this.shockTimer > 0) {
            ...
            this.freezeTimer = 0;
            this.shockTimer = 0;
            let superDmg = (this.maxHealth * 0.1);
            if (this.tier.id === 'BOSS') superDmg = Math.min(superDmg, 2000);
            superDmg += 150;
        ...
        if (this.freezeTimer > 0 && this.soakedTimer > 0) {
            ...
            let crushDmg = (this.maxHealth * 0.2);
            if (this.tier.id === 'BOSS') crushDmg = Math.min(crushDmg, 4000);
            crushDmg += 100;
                ...
                if (this.tier.id === 'BOSS') {
                    if (this.bossState === 'ENTER') {
                        ...
                        if (this.attackTimer > 60) {
                            // [Patch] Pass Tier ID for visual distinction
                            const bullet = new EnemyProjectile(this.positionX, this.positionY + 40, false, this.tier.id);
                    ...
                    if (this.tier.id === 'ROGUE') {
                        ...
                        // Aegis Logic: Hover Lower (Y=250) to avoid Boss overlap
                        const targetY = 250;
                        const dy = targetY - this.positionY;
                        this.positionY += dy * 0.05; // Smooth approach

                        // Hover motion
                        this.positionY += Math.sin(Date.now() / 500) * 0.5;

                        // Attack Logic (Heavy Shot)
                        this.attackTimer++;
                        if (this.attackTimer > 180) { // Slower fire rate (3 seconds)
                            // [Patch] Explicitly pass AEGIS type
                            const bullet = new EnemyProjectile(this.positionX, this.positionY + 40, false, 'AEGIS');
                            // Barrier Unit projectiles should be reflectable (default is)
                            // Maybe make them bigger/distinct? Handled in draw based on type? 
                            // Currently EnemyProjectile is generic.
                            activeEnemyProjectiles.push(bullet);
                            this.attackTimer = 0;
                            activeParticles.push(new ParticleEffect(this.positionX, this.positionY + 40, "#00d2d3", 6));
                        }
                    }
                    else {
...
class EnemyProjectile {
    // [Patch] Added `type` param (default 'NORMAL')
    constructor(x, y, isReflected = false, type = 'NORMAL') {
        this.x = x;
        this.y = y;
        ...
        // [Patch] Boss / Aegis Custom Props
        if (this.type === 'AEGIS') {
            this.speed = 5; // Slightly heavier/slower
            this.damage *= 1.5; // Heavier hit
        }
        else if (String(this.type).includes('BOSS')) { // Catch BOSS_SLIME etc
            this.size = 14; // Bigger
            this.damage *= 1.2;
            this.color = "#d35400";
        }
        ...
        if (this.isReflected) {
            ...
            for (const enemy of activeEnemies) {
                ...
                if (dist < enemy.size + this.size) {
                    enemy.takeDamage(500, true, 'reflected');
                    this.isAlive = false;
    ...
    draw(ctx) {
        ...
        // [Patch] Distinct Visuals
        if (this.type === 'AEGIS') {
            // AEGIS: Heavy Flashing Square (High Visibility)
            const pulse = (Math.sin(Date.now() / 150) + 1) * 0.5; // 0..1
            const sizeMod = 1 + pulse * 0.2;

            // Outer Glow (Neon Purple)
            ctx.shadowBlur = 20 + pulse * 10;
            ctx.shadowColor = "#e056fd";
            ctx.fillStyle = "#d05ce3";

            // Flash White at peak
            if (pulse > 0.8) ctx.fillStyle = "#fff";
        ...
        else if (String(this.type).includes('BOSS')) {
            // BOSS: Pulsing Magma Orb
            ctx.shadowBlur = 15;
            ctx.shadowColor = "#e74c3c";

            // Outer Halo
            ctx.fillStyle = `rgba(231, 76, 60, ${0.5 + Math.sin(Date.now() / 100) * 0.3})`;
            ctx.beginPath();
...
function handleAutoAttack() {
    ...
    loadouts.forEach(loadout => {
        ...
        if (fire) {
            // [Hardcore] Check Firing Enabled
            if (!engineState.isFiringEnabled) {
                return; // Ceasefire mode
            }

            // [Hardcore] Add Heat
            if (!engineState.isBeamActive && !engineState.isCoolingDown) {
                // Not in Cooldown, Not in Beam -> Accumulate Heat
                if (loadout.isMain) {
                    engineState.heat += engineState.heatPerShot;
                } else {
                    engineState.heat += engineState.heatPerShot * 0.5; // Sub weapon: half heat
                }
            }

            // [Hardcore] Overdrive Bonus (1.2x damage)
            // Only applies during the Overdrive Phase (5 seconds of max heat)
            if (engineState.isOverdrive) {
                finalConfig.damage *= 1.2;
            }
...
function spawnEnemy() {
    // [Patch] Aegis Spawn Rules v2
    // 1-4: 1x, 5-8: 2x, 9-10: 3x. Boss Active -> Stop.

    // Init Counter
    if (engineState._lastWaveAegis !== engineState.currentWaveNumber) {
        engineState._lastWaveAegis = engineState.currentWaveNumber;
        engineState._aegisSpawnedCount = 0;
    }

    const wave = engineState.currentWaveNumber;
    let maxAegis = 1;
    if (wave >= 9) maxAegis = 3;
    else if (wave >= 5) maxAegis = 2;

    const activeBoss = activeEnemies.some(e => e.isActive && !e.isDying && e.tier.id.includes('BOSS'));
    if (activeBoss) {
        // Boss Active: set remaining to 0 (by maxing out counter)
        engineState._aegisSpawnedCount = 99;
    }

    const activeAegis = activeEnemies.find(e => e.isActive && e.tier.id === 'AEGIS');
    ...
    if (!activeBoss && !activeAegis && engineState._aegisSpawnedCount < maxAegis) {
        ...
        if (isFirst || isRespawn) {
            activeEnemies.push(new EnemyUnit(ENEMY_TIERS.AEGIS));
            engineState._aegisSpawnedCount++;
            activeFloatingTexts.push(new FloatingText(GAME_SETTINGS.SCREEN_WIDTH / 2, 100, "⚠ BARRIER DEPLOYED", "#00d2d3", 24));
            return;
        }
    }
    ...
    // Standard Spawn Logic continues below...
...
function mainLoop() {
    ...
    if (!engineState.isPaused) {
        ...
        while (engineState.accumulator >= 1.0) {
            ...
            engineState.updateHeat(); // [Hardcore] Heat System
            ...
            // [Patch] Aegis Wall Blocking Logic
            ...
            if (activeAegis) {
                ...
                activeProjectiles.forEach(p => {
                    ...
                    // Full screen width, so no X check needed.
                    if (p.currentY < barrierY) {
                        // Destroy projectile
                        p.isAlive = false;
                        activeParticles.push(new ParticleEffect(p.currentX, barrierY, "#00d2d3", 2));
                    }
                });
            }
...
function renderScene() {
    ...
    // --- [Hardcore] Arc Reactor Heat Gauge ---
    gameContext.save();
    gameContext.translate(tx, ty);

    const heatRatio = Math.min(1.0, engineState.heat / engineState.maxHeat);
    const gaugeRadius = 50;

    // 1. Background Ring
    gameContext.beginPath();
    gameContext.arc(0, 0, gaugeRadius, 0, Math.PI * 2);
    ...
    const endAngle = startAngle + (Math.PI * 2 * heatRatio);

    let gaugeColor = "#00d2d3"; // Teal
    let glowColor = "#00d2d3";
    let gaugeWidth = 6;
    let gaugeText = "";

    if (engineState.isOverdrive) {
        // OVERDRIVE: Pulsating Red
        const pulse = Math.sin(Date.now() / 50) * 0.5 + 0.5;
        gaugeColor = `rgba(231, 76, 60, ${0.8 + pulse * 0.2})`;
        glowColor = "#e74c3c";
        gaugeWidth = 8 + pulse * 4;
        gaugeText = "OVERDRIVE";

        // Inner particles
        if (Math.random() < 0.3) {
            const pAngle = Math.random() * Math.PI * 2;
            const pDist = gaugeRadius * Math.random();
            gameContext.fillStyle = `rgba(231, 76, 60, ${Math.random()})`;
            gameContext.beginPath();
            gameContext.arc(Math.cos(pAngle) * pDist, Math.sin(pAngle) * pDist, Math.random() * 3, 0, Math.PI * 2);
            gameContext.fill();
        }
    } else if (engineState.isCoolingDown) {
        // COOLDOWN: Icy Blue
        gaugeColor = "#3498db";
        glowColor = "#2980b9";
        gaugeText = "COOLING...";
        ...
        // Normal Gradient
        if (heatRatio > 0.8) { gaugeColor = "#e67e22"; glowColor = "#d35400"; }
        else if (heatRatio > 0.5) { gaugeColor = "#f1c40f"; glowColor = "#f39c12"; }
    }

    // Draw Active Arc
    if (heatRatio > 0) {
        gameContext.beginPath();
        gameContext.arc(0, 0, gaugeRadius, startAngle, endAngle);
        gameContext.strokeStyle = gaugeColor;
        gameContext.lineWidth = gaugeWidth;
    ...
    if (gaugeText) {
        ...
        gameContext.fillText(gaugeText, 0, 85);
        gameContext.shadowBlur = 0;
    }
