...
/**
 * 数値を滑らかにカウントアップさせるアニメーション
 * @param {HTMLElement} element - 対象のDOM要素
 * @param {number} targetValue - 最終的な表示値
...
export function animateNumber(element, targetValue, duration = 500) {
    ...
    function update(currentTime) {
        ...
        if (progress < 1) {
            ...
            element.innerText = targetValue; // 最終値を確定
        }
    }
...
export function updateCrewHud() {
    ...
    if (!container) {
        ...
        container.style.cssText = "position:absolute; bottom:calc(230px + env(safe-area-inset-bottom)); right:10px; display:flex; flex-direction:column; gap:10px; z-index:45;";
...
export function refreshInventoryInterface() {
    ...
    const createSlot = (id, item, label, typeHint, dropTargetId) => {
        ...
        if (item) {
            ...
            // Check for custom iconImage first, then fallback to imgMap
...
export function showGameClearScreen() {
    ...
    // Continue Button (For Endless Mode)
    if (!engineState.isGameOver) {
        const continueBtn = document.createElement('button');
        continueBtn.innerHTML = "🚀 エンドレス突入 (CONTINUE)";
        continueBtn.style.cssText = "padding:12px 24px; background:#2ecc71; color:#fff; border:none; border-radius:4px; font-weight:bold; cursor:pointer; box-shadow: 0 0 10px #2ecc71;";
        continueBtn.onclick = () => {
            document.body.removeChild(overlay);
            engineState.isPaused = false; // Resume Game
        };
